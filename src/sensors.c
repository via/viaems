...
float fast_log2_approx(float val) {
    // Union for type punning (accessing float bits as int)
    union {
        float val;
        int32_t x;
    } u = { val };

    // Extract exponent: (u.x >> 23) & 255 gives the biased exponent.
    // Subtract 128 (approx) to get the actual exponent (e).
    // The snippet uses 128, but 127 is the correct bias for single precision.
    // The snippet also uses a polynomial approximation for the mantissa part.
    float log_2 = (float)(((u.x >> 23) & 255) - 128);

    // Isolate mantissa (normalized to [1, 2))
    u.x &= ~(255 << 23); // Clear exponent bits
    u.x += 127 << 23;    // Set exponent to 0 bias (value is now in [1, 2))

    float poly = ((-0.3358287811f) * u.val + 2.0f) * u.val - 0.65871759316667f;

    return (log_2 + poly);
}

float fast_ln_approx(float val) {
    return fast_log2_approx(val) * LN2_F;
}

float sensor_convert_thermistor(const struct sensor_config *conf, const float raw) {
  if (raw == 0.0f) {
    return 0.0f;
  }
  ...
  float logf_r = logf(r); //fast_ln_approx(r); //logf(r);
...
static float process_derivative(float previous_value,
                                float new_value) {
  return TICKRATE * (new_value - previous_value) /
         time_from_us(1000000 / platform_adc_samplerate());
}

static float process_lag_filter(float lag,
                                float old_value,
                                float new_value) {
  if (lag == 0.0f) {
    return new_value;
...
static void sensor_update_raw(struct sensor_state *s, const struct engine_position *d, timeval_t time, float raw) {
  ...
  if (detect_faults(s, raw) != FAULT_NONE) {
    ...
    case METHOD_LINEAR_WINDOWED:
      if (d->has_position && (d->rpm > 0)) {
        new_value = sensor_convert_linear_windowed(s, engine_current_angle(d, time), raw);
      } else {
        new_value = sensor_convert_linear(conf, raw);
      }
      break;
    ...
    out.derivative = process_derivative(s->output.value, new_value);
  }

  s->output = out;
}

static void update_single_freq_sensor(struct sensor_state *s, const struct engine_position *p, const struct freq_update *u) {
...
void sensor_update_freq(struct sensors *s, const struct engine_position *p, const struct freq_update *u) {
...
static void update_single_adc_sensor(struct sensor_state *s, const struct engine_position *p, const struct adc_update *u) {
...
void sensor_update_adc(struct sensors *s, const struct engine_position *p, const struct adc_update *u) {
...
bool sensor_has_faults(const struct sensor_values *s) {
  ...
  return (s->MAP.fault != FAULT_NONE) ||
         (s->BRV.fault != FAULT_NONE) || 
         (s->IAT.fault != FAULT_NONE) || 
         (s->CLT.fault != FAULT_NONE) || 
         (s->TPS.fault != FAULT_NONE) || 
         (s->FRT.fault != FAULT_NONE);
}
...
void knock_configure(struct knock_sensor *knock) {
  ...
  knock->freq = bucket;
  knock->w =
    2.0f * 3.14159f * (float)knock->freq / (float)knock->width;
...
static void knock_add_sample(struct knock_sensor *knock, float sample) {
  ...
  knock->n_samples += 1;
  float s =
    sample + knock->cr * 2 * knock->sprev - knock->sprev2;
  knock->sprev2 = knock->sprev;
  knock->sprev = s;

  if (knock->n_samples == 64) {
    knock->value =
      knock->sprev * knock->sprev +
      knock->sprev2 * knock->sprev2 -
...
void sensor_update_knock(struct sensors *s, const struct knock_update *update) {
  struct knock_sensor *knk =
    (update->pin == 0) ? &s->KNK1 : &s->KNK2;
...
START_TEST(check_sensor_convert_linear_windowed) {
  ...
  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 0, 2048.0f), 12.0, 0.01);

  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 10, 2500.0f), 12.0, 0.01);

  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 40, 2600.0f), 12.0, 0.01);
  ...
  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 60, 2800.0f), 12, 0.1);
  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 80, 2800.0f), 12, 0.1);
}
END_TEST
...
START_TEST(check_sensor_convert_linear_windowed_skipped) {
  ...
  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 120, 1000.0f), 12, 0.1);

  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 130, 1500.0f), 12, 0.1);
...
START_TEST(check_sensor_convert_linear_windowed_wide) {
  ...
  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 0, 2048), 12.0, 0.01);

  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 45, 2500), 12.0, 0.01);

  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 89, 2600), 12.0, 0.01);
  ...
  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 120, 1000), 12, 0.1);

  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 130, 1500), 12, 0.1);
...
START_TEST(check_sensor_convert_linear_windowed_offset) {
  ...
  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 10, 2500), 12.0, 0.01);

  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 40, 2600), 12.0, 0.01);

  /* Start of real window */
  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 50, 2700), 12, 0.1);

  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 80, 2400), 12, 0.1);

  /* End of window, should produce average of two prior samples */
  ck_assert_float_eq_tol(sensor_convert_linear_windowed(&state, 90, 2000), 2550, 0.1);
}
END_TEST
