...
static float calculate_airmass(float cylinder_cc, float ve, float map, float iat) {
 ...
 */
static float calculate_fuel_volume(float fuel_stp_density, float stoich_ratio, float airmass, float frt) {
...
static float calculate_tipin_enrichment(const struct config *config, struct tipin_state *state, timeval_t now, float tps, float tpsrate, int rpm) {

  float new_tipin_amount =
    interpolate_table_twoaxis(&config->tipin_enrich_amount, tpsrate, tps);

  /* Update status flag */
  if (state->active && !time_in_range(now,
                                      state->time,
...
struct calculated_values calculate_ignition_and_fuel(const struct config *config, const struct engine_update *update, struct calculations *state) {
  ...
  float timing_advance = interpolate_table_twoaxis(&config->timing, pos->rpm, map);
  float dwell_us = 0.0f;
  ...
  float lambda = interpolate_table_twoaxis(&config->commanded_lambda, pos->rpm, map);
  ...
  float tipin = calculate_tipin_enrichment(config, &state->tipin, update->current_time, tps, tpsrate, pos->rpm);

  float airmass_per_cycle = calculate_airmass(config->fueling.cylinder_cc, ve, map, iat);

  float fuel_vol_at_stoich =
    calculate_fuel_volume(config->fueling.density_of_fuel, config->fueling.fuel_stoich_ratio, airmass_per_cycle, frt);

  float fuelvol_per_cycle = fuel_vol_at_stoich / lambda;

  float raw_pw_us =
    (fuelvol_per_cycle +
     (tipin / 1000)) / /* Tipin unit is mm^3 */
    config->fueling.injector_cc_per_minute *
    60000000 /                           /* uS per minute */
    config->fueling.injections_per_cycle; /* This many pulses */

  float pwc = interpolate_table_oneaxis(&config->injector_pw_correction, raw_pw_us / 1000.0f);
  ...
  bool fuel_overduty_cut_enabled = time_from_us(commanded_pw_us) >= max_allowable_pw;
...
START_TEST(check_calculate_airmass) {
  ...
  ck_assert_float_eq_tol(calculate_airmass(cylinder_cc, 100, 70, 0), 0.7 * airmass, 0.001);

  /* 80 VE should be 80% of first airmass */
  ck_assert_float_eq_tol(calculate_airmass(cylinder_cc, 80, 100, 0), 0.8 * airmass, 0.001);
}
END_TEST
...
START_TEST(check_ign_fuel_calcs) {
  ...
  struct calculated_values results = calculate_ignition_and_fuel(&default_config, &update, &state);
...
START_TEST(check_calculate_tipin_newevent) {
  ...
  ck_assert_float_eq_tol(calculate_tipin_enrichment(&tipin_test_config, &state, time, 0, 0, 100), 0, 0.001);
  ck_assert_float_eq_tol(
    calculate_tipin_enrichment(&tipin_test_config, &state, time, 100, 100, 100), 2000.0, 0.001);
  /* At 100 rpm, should last 1 ms */

  time = time_from_us(900);
  ck_assert_float_eq_tol(calculate_tipin_enrichment(&tipin_test_config, &state, time, 0, 0, 100), 2000.0, 0.001);

  time = time_from_us(1005);
  ck_assert_float_eq_tol(calculate_tipin_enrichment(&tipin_test_config, &state, time, 0, 0, 100), 0.0, 0.001);
}
END_TEST
...
START_TEST(check_calculate_tipin_overriding_event) {
  ...
  ck_assert_float_eq_tol(calculate_tipin_enrichment(&tipin_test_config, &state, time, 0, 0, 100), 0, 0.001);
  ck_assert_float_eq_tol(calculate_tipin_enrichment(&tipin_test_config, &state, time, 100, 50, 100), 1000, 0.001);

  /* At 100 rpm, should last 1 ms */
  time = time_from_us(500);
  /* Milder tipin should not affect anything */
  ck_assert_float_eq_tol(calculate_tipin_enrichment(&tipin_test_config, &state, time, 100, 20, 100), 1000, 0.001);

  time = time_from_us(1005);
  ck_assert_float_eq_tol(calculate_tipin_enrichment(&tipin_test_config, &state, time, 0, 0, 100), 0.0, 0.001);

  /* Test a higher overriding event */
  ck_assert_float_eq_tol(calculate_tipin_enrichment(&tipin_test_config, &state, time, 100, 20, 100), 400, 0.001);
  ...
  ck_assert_float_eq_tol(calculate_tipin_enrichment(&tipin_test_config, &state, time, 0, 0, 100), 1000, 0.001);
