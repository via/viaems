 ...
 * a trailing CRC and NUL delimiter:
 *
 * ---------------------------------
 * | [ LEN | MESSAGE | CRC ] | NUL |
 * ---------------------------------
 ...
 *    - MESSAGE is the PDU
 *    - CRC is a CRC-32/HDLC, the same used for ethernet. It is computed across
 * the bytes in MESSAGE NUL is a single NUL
 */
...
static bool cobs_flush(struct cobs_encoder *enc,
                       stream_write_fn write,
                       void *arg) {
...
static bool stream_message_write(struct stream_message_writer *msg,
  ...
  if (msg->consumed == msg->length) {
    /* finish by frame with the crc and flushing the cobs decoder, which
     * produces a NUL delimiter */
...
static bool stream_message_reader_new(struct stream_message_reader *msg,
  ...
  if (!cobs_decode(
        &msg->cobs, sizeof(received_length), received_length, read, arg)) {
    return false;
  }
  msg->length =
    (uint16_t)received_length[0] | ((uint16_t)received_length[1] << 8);
...
static bool stream_message_read(struct stream_message_reader *msg,
  ...
  if (msg->bytes_read == msg->length) {
    /* Read CRC */
    uint8_t crc_bytes[4];
    if (!cobs_decode(&msg->cobs, sizeof(crc_bytes), crc_bytes, read, arg)) {
      return false;
    }
    uint32_t received_crc =
      (uint32_t)crc_bytes[0] | ((uint32_t)crc_bytes[1] << 8) |
...
bool platform_message_writer_write(struct console_tx_message *msg,
                                   const uint8_t *data,
                                   size_t length) {
  (void)msg;
  timeval_t fail_after = current_time() + time_from_us(10000);
...
bool platform_message_reader_read(struct console_rx_message *msg,
                                  uint8_t *data,
                                  size_t length) {
  (void)msg;

  if (!stream_message_read(
        &msg_reader, blocking_read_timeout, &blocking_read_arg, length, data)) {
    if (!msg_reader.eof) {
      read_stream_synchronized = false;
    }
    return false;
...
void platform_message_reader_abort(struct console_rx_message *rxmsg) {
  (void)rxmsg;
}
...
START_TEST(test_cobs_encode) {
  ...
  }
  {
    ...
    // First overhead before 255, then another that is both less 
    // than 255 later and greater than 255 into the scratch
    large[172] = 0;
    large[384] = 0;
    ...
    ck_assert_int_eq(ctx.size, 602);
    ck_assert_int_eq(ctx.buffer[0], 173);
    ck_assert_int_eq(ctx.buffer[173], 212);
    ck_assert_int_eq(ctx.buffer[385], 216);
  }
  {
    ...
    for (unsigned i = 0; i < ctx.size; i++) {
      if (expected[i] != ctx.buffer[i]) {
        ck_abort_msg("expected[%d] == %d, but ctx[%d] == %d",
                     i,
                     expected[i],
                     i,
                     ctx.buffer[i]);
...
START_TEST(test_stream_message_write) {
  ...
  result =
    stream_message_write(&msg, test_write_fn, &ctx, sizeof(msg_text), msg_text);
  ck_assert(result);
