...
bool stream_message_reader_new(struct stream_message_reader *msg,
...

  {
    ...
    if (!cobs_decode(
          &msg->cobs, sizeof(received_crc), received_crc, msg->read, arg)) {
      return false;
    }
    msg->crc = (uint32_t)received_crc[0] | ((uint32_t)received_crc[1] << 8) |
...
static const uint8_t large_array_encoded[] = {
  ...
  0xb9, 0x75, 0x35, 0xd8, 0x50, 0xfc, 0x02
};
...
START_TEST(test_stream_message_write) {
  ...
  bool result =
    stream_message_writer_new(&msg, test_write_fn, &ctx, sizeof(msg_text), crc);
  ck_assert(result);
