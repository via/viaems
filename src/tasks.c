...
static bool determine_fuel_pump_state(struct tasks *state, const struct engine_update *update) {

  const struct engine_position *d = &update->position;

  /* If engine is actively turning, fuel pump is on */
  if (d->has_rpm && time_in_range(update->current_time, d->time, d->valid_until)) {
  ...
  if (time_diff(update->current_time, state->fuelpump_last_valid) < time_from_us(4000000)) {
    return true;
  } else {
...
static float handle_boost_control(const struct boost_control_config *config, const struct engine_update *u) {
  float duty;
  ...
  if (map < config->enable_threshold_kpa) {
    ...
    duty = interpolate_table_oneaxis(&config->pwm_duty_vs_rpm,
                                     u->position.rpm);
  }
  return duty;
}
...
static cel_state_t determine_next_cel_state(const struct cel_config *config, const struct engine_update *u) {
  ...
  bool lean_in_boost =
    (map > config->lean_boost_kpa) &&
    (ego > config->lean_boost_ego);
...
static bool handle_check_engine_light(const struct cel_config *config, struct tasks *state, const struct engine_update *u) {
  ...
  return determine_cel_pin_state(state->cel_state, u->current_time, state->cel_time);
}


void run_tasks(struct viaems *viaems, const struct engine_update *update, struct platform_plan *plan) {
  plan_set_gpio(plan, viaems->config->fueling.fuel_pump_pin, 
      determine_fuel_pump_state(&viaems->tasks, update));

  plan_set_gpio(plan, viaems->config->cel.pin, 
      handle_check_engine_light(&viaems->config->cel, &viaems->tasks, update));

  plan_set_pwm(plan, viaems->config->boost_control.pin,
