...
static void even_tooth_trigger_update(struct decoder *d, timeval_t t) {
  ...
  if (d->current_triggers_rpm < MAX_TRIGGERS + 1) {
...
static uint32_t missing_tooth_average_rpm(const struct decoder *d) {
...
static void decode_missing_no_sync(struct decoder *d,
  ...
  d->rpm = missing_tooth_average_rpm(d);
...
static struct timed_callback test_trigger_callback;
...
START_TEST(check_missing_tooth_rpm_after_gap) {
  ...
  ck_assert(config.decoder.state == DECODER_SYNC);
  ck_assert_int_eq(
    config.decoder.rpm,
