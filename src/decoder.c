...
static void even_tooth_trigger_update_rpm(struct decoder *state) {
  ...
  unsigned int rpm_window_size =
    even_tooth_rpm_window_size(state->current_triggers_rpm, conf->rpm_window_size);

  if (rpm_window_size > 1) {
    /* We have at least two data points to draw an rpm from */
    out->rpm = rpm_from_time_diff(state->times[0] - state->times[rpm_window_size],
                                conf->degrees_per_trigger * rpm_window_size);

    if (out->rpm > 0) {
      unsigned int delta =
        (out->rpm > out->tooth_rpm) ? out->rpm - out->tooth_rpm : out->tooth_rpm - out->rpm;
...
static void decode_even_no_sync(struct decoder *state, struct trigger_event *ev) {
...
static uint32_t missing_tooth_rpm(struct decoder *state) {
  ...
  degrees_t rpm_degrees = state->config->degrees_per_trigger * (last_tooth_missing ? 2 : 1);
...
static void missing_tooth_trigger_update(struct decoder *state, timeval_t t) {
  ...
  if (state->state == DECODER_RPM) {
    ...
    timeval_t expected_time = time_from_rpm_diff(state->output.rpm, expected_gap);
...
static uint32_t missing_tooth_average_rpm(const struct decoder *state) {
  ...
  if (state->current_triggers_rpm >= conf->num_triggers) {
    if (state->triggers_since_last_sync == 0) {
      return rpm_from_time_diff(state->times[0] - state->times[conf->num_triggers - 1],
...
static void decode_missing_with_camsync(struct decoder *state,
  ...
  if (ev->type == TRIGGER) {
    missing_tooth_trigger_update(state, ev->time);
    if ((state->state == DECODER_SYNC) && (state->triggers_since_last_sync == 0)) {
...
struct engine_position decoder_get_engine_position(const struct decoder *state) {
  struct engine_position result = state->output;
  result.has_rpm = (state->state == DECODER_RPM) || (state->state == DECODER_SYNC);
  return result;
}

bool engine_position_is_synced(const struct engine_position *d, timeval_t at_time) {
  return d->has_position && time_before(at_time, d->valid_until + 1600);
}

degrees_t engine_current_angle(const struct engine_position *d, timeval_t at_time) {
...
static void validate_decoder_sequence(struct decoder *state, struct decoder_event *ev) {
  for (; ev; ev = ev->next) {
    struct trigger_event e = {.time = ev->time, .type = ev->trigger == 0 ? TRIGGER : SYNC };
    decoder_update(state, &e);
    ck_assert_msg(
      (state->state == ev->state) &&
        (state->output.has_position == ev->valid),
...
static void prepare_36minus1_crank_wheel_plus_cam_decoder(struct decoder *dstate) {
  static const struct decoder_config dconfig = {
...
START_TEST(check_update_rpm_sufficient_points) {
  ...
  even_tooth_trigger_update_rpm(&d);
  ck_assert_int_eq(d.output.rpm, rpm_from_time_diff(100, config.degrees_per_trigger));
}
END_TEST
