...
static void populate_plan_from_events(struct platform_plan *plan, struct output_event_schedule_state *events) {
  plan->n_events = 0;
  for (int i = 0; i < MAX_EVENTS; i++) {
    {
      struct schedule_entry *start = &events[i].start;
      if ((start->state == SCHED_SCHEDULED) &&
           time_in_range(start->time, plan->schedulable_start, plan->schedulable_end)) {
...

    {
      ...
      if ((stop->state == SCHED_SCHEDULED) &&
           time_in_range(stop->time, plan->schedulable_start, plan->schedulable_end)) {
