...
static bool event_has_fired(struct output_event_schedule_state *ev) {
  return (ev->start.state == SCHED_FIRED) &&
 ...
 * to schedule */
static bool sched_entry_enable(struct schedule_entry *en, timeval_t time, timeval_t earliest_schedulable_time) {

  if ((en->state == SCHED_SUBMITTED) ||
      (en->state == SCHED_FIRED)) {
    return false;
  }
...
void invalidate_scheduled_events(struct output_event_schedule_state *evs, int n) {
  for (int i = 0; i < n; ++i) {
...
static void schedule_output_event_safely(struct output_event_schedule_state *ev,
  ...
  if (event_is_unscheduled(ev)) {
    ...
    int success = sched_entry_enable(&ev->start, newstart, earliest_schedulable_time);
...
static bool schedule_ignition_event(const struct config *config,
  ...
  degrees_t firing_angle = clamp_angle(clamp_angle(ev->config->angle - advance, 720) -
      d->last_trigger_angle + config->decoder.offset, 720);

  timeval_t stop_time = d->time + time_from_rpm_diff(d->tooth_rpm, firing_angle);
  ...
  schedule_output_event_safely(ev, earliest_schedulable_time, start_time, stop_time);
...
static bool schedule_fuel_event(const struct config *config,
  ...
  degrees_t firing_angle =
    clamp_angle(ev->config->angle - pos->last_trigger_angle + config->decoder.offset, 720);

  timeval_t stop_time = pos->time + time_from_rpm_diff(pos->tooth_rpm, firing_angle);
...
START_TEST(check_schedule_ignition) {
  ...
  ck_assert_int_eq(ev.stop.time - ev.start.time,
                   time_from_us(dwell));
  ck_assert_int_eq(ev.stop.time,
                   time_from_rpm_diff(pos.rpm,
                                               ev.config->angle + 
                                               default_config.decoder.offset - 
                                               advance));
}
END_TEST
...
START_TEST(check_schedule_ignition_reschedule_active_later) {
  ...
  ck_assert_int_eq(
    ev.stop.time,
    time_from_rpm_diff(pos.rpm, ev.config->angle + default_config.decoder.offset));
}
END_TEST
