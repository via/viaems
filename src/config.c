...
int config_valid() {
  ...
  if (config.injector_pw_compensation &&
      !table_valid_oneaxis(config.injector_pw_compensation)) {
    return 0;
  }

  if (config.commanded_lambda && !table_valid_twoaxis(config.commanded_lambda)) {
    return 0;
  }

  if (config.tipin_enrich_amount && !table_valid_twoaxis(config.tipin_enrich_amount)) {
    return 0;
  }
