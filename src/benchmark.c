...
static void report_benchmark(const char *name, const struct benchmark_results results) {
...
static uint32_t do_schedule_ignition_from_unscheduled() {
  ...
  uint64_t start = cycle_count();
  schedule_events(&default_config, &bench_calcs, &schedule_bench_update.position, &ev, 1, 0);
  uint64_t end = cycle_count();
...
static uint32_t do_schedule_ignition_earlier() {
  ...
  uint64_t start = cycle_count();
  schedule_events(&default_config, &newcalcs, &schedule_bench_update.position, &ev, 1, 0);
  uint64_t end = cycle_count();
  assert(ev.start.state == SCHED_SCHEDULED);
...
static struct benchmark_results do_missing_tooth_sequence(uint32_t count) {
  ...
  for (size_t tooth = 0, i = 0; i < count;) {
    ...
    if (cam_sync) {
      ...
      decoder_update(&decoder, &(struct trigger_event){ .time = time, .type = SYNC });
    ...
    if (missing) {
      ...
      uint32_t start = cycle_count();
      decoder_update(&decoder, &(struct trigger_event){ .time = time, .type = TRIGGER });
      uint32_t cycles = cycle_count() - start;
      accumulator += cycles;
      if (cycles < results.min) {
        results.min = cycles;
...
int start_benchmarks() {
  ...
  report_benchmark("Fuel Calculations", run_benchmark(do_calculation_bench, 1000));
  report_benchmark("Sensors - Thermistor", run_benchmark(do_sensor_single_therm, 1000));
  report_benchmark("Sensors - Linear", run_benchmark(do_sensor_single_linear, 1000));
  report_benchmark("Sensors - Process All ADC", run_benchmark(do_sensor_all_adc_calcs, 1000));
  ...
  report_benchmark("Scheduler - Ign schedule", run_benchmark(do_schedule_ignition_from_unscheduled, 1000));
  report_benchmark("Scheduler - Ign (earlier)", run_benchmark(do_schedule_ignition_earlier, 1000));
  report_benchmark("Scheduler - Ign (later)", run_benchmark(do_schedule_ignition_later, 1000));

  report_benchmark("Scheduler - Fuel schedule", run_benchmark(do_schedule_fuel_from_unscheduled, 1000));
  report_benchmark("Scheduler - Fuel (later)", run_benchmark(do_schedule_fuel_later, 1000));
  report_benchmark("Scheduler - Fuel (earlier)", run_benchmark(do_schedule_fuel_earlier, 1000));
  report_benchmark("Scheduler - Deschedule", run_benchmark(do_schedule_deschedule, 1000));
  report_benchmark("Engine Loop - Happy path", run_benchmark(do_viaems_reschedule_normal, 1000));

  report_benchmark("Decoder - missing+camsync", do_missing_tooth_sequence(1000));
  puts("\r\nDone!\r\n");
