...
#ifdef DBG_TICK_OUT
  /* Enable PB0 as TIM8 CH2N as a 50% duty cycle square wave with the rising
...
void TIM2_IRQHandler(void) {
  ...
  if ((TIM2->SR & TIM_SR_CC1OF) || (TIM2->SR & TIM_SR_CC2OF)) {
    ...
    decoder_update(&stm32f4_viaems.decoder, &(struct trigger_event){ .time = cc2, .type = cc2_type });
    decoder_update(&stm32f4_viaems.decoder, &(struct trigger_event){ .time = cc1, .type = cc1_type });
  } else {
    if (cc1_fired) {
      decoder_update(&stm32f4_viaems.decoder, &(struct trigger_event){ .time = cc1, .type = cc1_type });
    }
...
void DMA2_Stream1_IRQHandler(void) {
  ...
  if (DMA2->LISR & DMA_LISR_TCIF1) {
    ...
    update.current_time = output_buffers[stm32_current_buffer()].plan.schedulable_start;
...
static void setup_scheduled_outputs(void) {
  ...
  for (int i = 0; i < MAX_EVENTS; ++i) {
    if (stm32f4_viaems.config->outputs[i].inverted && stm32f4_viaems.config->outputs[i].type != DISABLED_EVENT) {
