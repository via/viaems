...
static void retire_output_buffer(struct output_buffer *buf) {
  for (int i = 0; i < buf->plan.n_events; i++) {
    ...
    platform_output_slot_unset(buf->slots, offset_from_start, entry->pin, entry->val);
...
static void populate_output_buffer(struct output_buffer *buf) {
  for (int i = 0; i < buf->plan.n_events; i++) {
    ...
    timeval_t offset_from_start = entry->time - buf->plan.schedulable_start;
    platform_output_slot_set(buf->slots, offset_from_start, entry->pin, entry->val);
...
void stm32_buffer_swap(struct viaems *viaems, const struct engine_update *update) {
