...
static void execute_plan(struct platform_plan *plan) {
  qsort(plan->schedule, plan->n_events, sizeof(struct schedule_entry *), schedule_entry_compare);
  for (int i = 0; i < plan->n_events; i++) {
...
static void handle_replay_events(timeval_t until_time) {
  ...
  while (time_before(current_event.time, until_time)) {
    ...
    case END_EVENT:
      ...
      break;
    case TRIGGER_EVENT:
      decoder_update(&hosted_viaems.decoder, &(struct trigger_event){ .time = current_event.time, .type = current_event.trigger == 0 ? TRIGGER : SYNC });
      break;
    case NO_EVENT:
      break;
    case ADC_EVENT:
...
int main(int argc, char *argv[]) {
  ...
  sensor_update_adc(&hosted_viaems.sensors, &(struct engine_position){ 0 }, &current_adc);
