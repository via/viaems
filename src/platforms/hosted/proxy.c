...
static void read_callback(struct libusb_transfer *xfer) {
  ...
  libusb_fill_bulk_transfer(xfer, devh, USB_IN_EP, xfer->buffer, xfer->length, read_callback, NULL, 1000);
...
static int tx_loop(void *ptr) {
  ...
  do {
    ...
    int actual_length;
    while (libusb_bulk_transfer(devh, USB_OUT_EP, rxbuf, len,
          &actual_length, 0) < 0);
  } while (true);
  return 0;
}
...
int main(void) {
    ...
    struct {
     ...
     for (int i = 0; i < 4; i++) {
       ...
       libusb_fill_bulk_transfer(transfers[i].xfer, devh, 0x81, transfers[i].buffer, sizeof(transfers[i].buffer), read_callback, NULL, 1000);
