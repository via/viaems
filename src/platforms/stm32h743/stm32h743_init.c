...
static void setup_clocks() {
  ...
  RCC->PLL3DIVR =
    _VAL2FLD(RCC_PLL3DIVR_N3, 5) | /* N = 6, provides 48 MHz internally */
...
static void configure_usart1() {
  ...
  GPIOB->MODER |=
    GPIO_MODER_MODE6_1 | GPIO_MODER_MODE7_1; /* B6/B7 in AF mode */
  GPIOB->AFR[0] |=
    _VAL2FLD(GPIO_AFRL_AFSEL6, 7) | _VAL2FLD(GPIO_AFRL_AFSEL7, 7);
}
