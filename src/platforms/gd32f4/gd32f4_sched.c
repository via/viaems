...
void TIMER1_IRQHandler(void) {
  ...
  if (cc0_fired && cc1_fired && time_before(cc1, cc0)) {
    decoder_update(&gd32f4_viaems.decoder, &(struct trigger_event){ .time = cc1, .type = cc1_type });
    decoder_update(&gd32f4_viaems.decoder, &(struct trigger_event){ .time = cc0, .type = cc0_type });
  } else {
    if (cc0_fired) {
      decoder_update(&gd32f4_viaems.decoder, &(struct trigger_event){ .time = cc0, .type = cc0_type });
    }
  ...
  if (TIMER_INTF(TIMER1) & TIMER_INTF_CH2IF) {
    ...
    struct engine_position pos = decoder_get_engine_position(&gd32f4_viaems.decoder);
...
static void setup_scheduled_outputs(void) {
  ...
  for (int i = 0; i < MAX_EVENTS; ++i) {
    if (default_config.outputs[i].inverted && default_config.outputs[i].type != DISABLED_EVENT) {
