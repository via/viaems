...
static struct spsc_queue engine_update_queue = {
  .size = 4,
};
static struct viaems_console_EngineUpdate engine_update_msgs[4];
...
static void console_process_request(struct console_rx_message *rxmsg, struct config *config) {
  ...
  bool reset_to_bootloader = false;
  switch (console_message.request.which_request) {
    ...
    case PB_TAG_viaems_console_Request_resettobootloader:
      /* Don't immediately reset so that we can issue a response */
      reset_to_bootloader = true;
      console_message.which_msg = PB_TAG_viaems_console_Message_response;
      console_message.response.which_response = PB_TAG_viaems_console_Response_resettobootloader;
      break;
