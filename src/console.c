...
static void render_description_field(CborEncoder *enc, const char *description) {
...
static size_t console_feed_line_keys(uint8_t *dest, size_t bsize) {
  ...
  cbor_encoder_create_array(
    &top_encoder, &key_list_encoder, count);
...
void record_engine_update(const struct viaems *viaems,
  ...
  update->map = eng_update->sensors.AAP.value;
  ...
  update->sync = engine_position_is_synced(&eng_update->position, eng_update->current_time);
...
static void render_boost_control(struct console_request_context *ctx,
                                 void *ptr) {
  struct boost_control_config *boost_control = (struct boost_control_config *)ptr;
  render_uint32_map_field(
...
static void render_cel(struct console_request_context *ctx, void *ptr) {
  ...
  render_uint32_map_field(
    ctx, "pin", "GPIO pin for CEL output", &cel->pin);
  render_float_map_field(ctx,
                         "lean-boost-kpa",
...
static void render_trigger_input_object(struct console_request_context *ctx, void *ptr) {
  ...
  render_enum_map_field(
    ...
    (const struct console_enum_mapping[]){
      { NONE, "none" }, { FREQ, "freq" }, { TRIGGER, "trigger" }, { SYNC, "sync" }, { 0, NULL } },
    &type);
...
static void render_trigger_list(struct console_request_context *ctx, void *ptr) {
  struct trigger_input *trigger_inputs = (struct trigger_input *)ptr;
  for (int i = 0; i < 4; i++) {
    render_map_array_field(ctx, i, render_trigger_input_object, &trigger_inputs[i]);
  }
}
...
static void console_toplevel_request(struct console_request_context *ctx,
  ...
  render_map_map_field(ctx, "boost-control", render_boost_control, &config->boost_control);
  render_map_map_field(ctx, "check-engine-light", render_cel, &config->cel);
  render_array_map_field(ctx, "trigger", render_trigger_list, &config->trigger_inputs);
...
static void console_toplevel_types(struct console_request_context *ctx,
                                   void *ptr) {
  struct config *config = ptr;
  render_map_map_field(ctx, "sensor", render_sensor_object, &config->sensors.IAT);
  render_map_map_field(
...
static void console_request_get(CborEncoder *enc, CborValue *pathlist, struct config *config) {
  struct console_request_context ctx = {
    .type = CONSOLE_GET,
    .response = enc,
    .path = pathlist,
...
static void console_process_request(CborValue *request, CborEncoder *response, struct config *config) {
